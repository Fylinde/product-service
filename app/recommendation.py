...
from app.models.recommendation import UserInteraction, ProductModel
from app.models.vendor import VendorModel
def recommend_products(user_id: int, db: Session):
    # Fetch user interaction data
    user_data = db.query(UserInteraction).filter(UserInteraction.user_id == user_id).all()
...
def recommend_products_user_based(user_id: int, db: Session):
    ...
    similar_users = db.query(UserInteraction.user_id).filter(UserInteraction.product_id.in_(
    ...
    recommended_product_ids = db.query(UserInteraction.product_id).filter(
        UserInteraction.user_id.in_(similar_users)).distinct()
...
def recommend_products_item_based(user_id: int, db: Session):
    ...
    similar_product_ids = db.query(UserInteraction.product_id).filter(UserInteraction.user_id.in_(
        db.query(UserInteraction.user_id).filter(UserInteraction.product_id.in_(
